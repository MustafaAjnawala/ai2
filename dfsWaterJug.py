def solveWaterJugProblem(capacity_jug1, capacity_jug2, desired_quantity):
    stack = []
    stack.append((0, 0, []))  # Initial state: both jugs empty, with an empty path
    visited = set()

    while stack:
        current_state = stack.pop() #removes ele from the end of the list(Popping in stack)
        ...
        for state in next_states:
            stack.append((state[0], state[1], path.copy()))#adds the ele at the end of the list
